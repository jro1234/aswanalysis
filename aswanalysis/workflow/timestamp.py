...
def timestamp_to_workflowtime(tstamp, tstart):
    if not isinstance(tstamp, float):
        t = timestamp_to_sinceepoch(tstamp)
    else:
        t = tstamp

    return t - tstart if isinstance(t, (int,float)) else t
        ...
        for line in f:
            ...
            for k, stamps in timestamps.items():
                ...
                if c.search(line):
                    ...
                    and ll[-2].split('-')[0].startswith('20'):
                        stamp = '_'.join([ll[-2], stamp])

                    elif ll[1].startswith("20") and ll[2][:2].isdigit():
                        stamp = "_".join([ll[1], ll[2]])
    ...
    for filename in filenames:
        ...
        if isinstance(filename, (tuple,list)):
            ...
            tasks_timestamps.append({
                k:v
                for d in _tasks_timestamps + _errors_timestamps
